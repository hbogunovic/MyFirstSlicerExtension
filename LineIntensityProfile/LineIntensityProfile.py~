...
class LineIntensityProfileWidget(ScriptedLoadableModuleWidget):
  ...
  def setup(self):
    ...
    self.inputRuler.setToolTip( "Select the ruler for intensity profile." )
...
class LineIntensityProfileLogic(ScriptedLoadableModuleLogic):
  ...
  def run(self,inputVolume1,inputVolume2,inputRuler):
    ...
    print('Before running the algorithm')
    ...
    volume2samples = []
    if inputVolume1:
      volume1samples = self.probeVolume(inputVolume1, inputRuler)
    if inputVolume2:
      volume2samples = self.probeVolume(inputVolume2, inputRuler)        
    
    imageSamples = [volume1samples, volume2samples]
  ...
  def probeVolume(self, volumeNode, rulerNode):
    ...
    line.SetPoint1(p1ijk[0],p1ijk[1],p1ijk[2])
